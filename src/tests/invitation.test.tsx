...
jest.mock('../components/InvitationCard', () => {
  return function MockInvitationCard({ status }: InvitationCardProps) {
    return <div data-testid={`invitation-card-${status.toLowerCase()}`}>{status}</div>;
  };
});
...
describe('Invitation Component', () => {
  ...
  test('renders four InvitationCard components', () => {
    ...
    expect(screen.getByTestId('invitation-card-invitations')).toBeInTheDocument();
  });
  ...
  test('renders the search section', () => {
    expect(screen.getByText('Search for Invitation Status')).toBeInTheDocument();
    expect(screen.getByPlaceholderText('Search by email or name of the invitee')).toBeInTheDocument();
