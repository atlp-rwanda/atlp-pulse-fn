...
function DashboardCards() {
    ...
    const { loading: getTicketsDataLoading } = useQuery(GET_TICKETS, {
        onCompleted: (data) => {
            ...
            const activeCount = tickets.filter((ticket: { status: string; }) => ticket.status !== 'closed').length;
            const closedCount = tickets.filter((ticket: { status: string; }) => ticket.status === 'closed').length;
    ...
    const { loading: getInvitationsDataLoading } = useQuery(GET_ALL_INVITATIONS, {
        onCompleted: (data) => {
            ...
            // Count active and closed tickets
            const acceptedInvitationsCount = invitations.filter((invite: { status: string; }) => invite.status === 'accepted').length;
            const pendingInvitationsCount = invitations.filter((invite: { status: string; }) => invite.status === 'pending').length;
            const declinedInvitationsCount = invitations.filter((invite: { status: string; }) => invite.status === 'cancelled').length;
    ...
    const InvitationChartData = {
        labels: ['Accepted Invitations', 'Pending Invitations', 'Declined Invitations'],
        datasets: [
            {
                label: 'Invitations',
                data: [acceptedInvitationsCount, pendingInvitationsCount, declinedInvitationsCount],
                    ...
                    {!getAllTeamsDataLoading && (
                        <>
                            <div className="flex items-end xmd:items-center gap-1 xmd:gap-2">
                                <RiTeamFill className="w-8 h-8 text-primary" /> {/* Increased icon size */}
