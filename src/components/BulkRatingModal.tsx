...
function BulkRatingModal({
  ...
  const [
    getUserCohorts,
    { data: cohorts, loading: loadingCohorts, error: cohortsError },
  ] = useLazyQuery(GET_USER_COHORTS, {
    variables: {
  ...
  const [
    getRatingsByCohort,
    { data: ratings, loading: loadingRatings, error: ratingsError },
  ] = useLazyQuery(GET_RATINGS_BY_COHORT, {
    fetchPolicy: 'network-only',
  });
  const [
    getTeamsByCohort,
    { data: teams, loading: loadingTeams, error: teamsError },
  ] = useLazyQuery(GET_TEAMS_BY_COHORT, {
    fetchPolicy: 'network-only',
  });
  const [
    addRatingsByFile,
    { data: bulkRatings, loading: loadingBulkRatings, error: bulkRatingsError },
  ] = useMutation(ADD_RATINGS_BY_FILE);
  ...
  const downloadTeamFile = async (e: any) => {
    try {
      ...
      const team = teams.getTeamsByCohort.find(
          ...
          >
            <div className="flex flex-col">
              ...
              <select
                ...
                className="p-2 my-2 text-sm text-black dark:text-white rounded-lg bg-white dark:bg-dark border-2 border-primary"
              ...
              >
                ...
                {cohorts && cohorts.getUserCohorts.length
                  ...
                  : ''}
                {loadingCohorts ? (
                  <option data-testid="cohort-loading-option">
                    loading ...
                  </option>
                ) : (
                  ''
                ...
                {cohortsError ? (
                  <option data-testid="cohort-error-option">
                    No cohorts found
                  </option>
                ) : (
                  ''
              ...
              >
                ...
                {ratings && !ratings.getRatingsByCohort.length ? (
                  <option data-testid="sprint-default-option" value={1}>
                    Sprint 1
                  </option>
                ) : (
                  ''
                ...
                {ratings && ratings.getRatingsByCohort.length
                  ? [...ratings.getRatingsByCohort].map((rating: any) => (
                      <option
                        data-testid={`sprint-option-${rating.id}`}
                        key={rating.id}
                        value={rating.sprint}
                      >
                ...
                {ratings && ratings.getRatingsByCohort.length ? (
                  <option
                    data-testid="sprint-new-option"
                    value={[...ratings.getRatingsByCohort].pop().sprint + 1}
                  >
                    Sprint {[...ratings.getRatingsByCohort].pop().sprint + 1}
                  </option>
                ) : (
                  ''
                )}
                {loadingRatings ? (
                  <option data-testid="sprint-loading-option">
                    Loading...
                  </option>
                ) : (
                  ''
                ...
                {ratingsError ? (
                  <option data-testid="sprint-error-option">
                    No sprints found...
                  </option>
                ) : (
                  ''
            ...
            <div className="flex flex-col gap-2">
              ...
              <div>
                {bulkRatings &&
                bulkRatings.addRatingsByFile.RejectedRatings.length > 0 ? (
                  <div className="my-1 overflow-x-auto">
                    <table className="table-fixed min-w-full">
                      ...
                      <thead className="border-b bg-neutral-700 border-neutral-400">
                        <tr>
                          <th scope="col" className="text-left py-4 px-2">
                            Email
                          </th>
                          <th scope="col" className="text-left py-4 px-2">
                            Quantity
                      ...
                      <tbody>
                        {bulkRatings.addRatingsByFile?.RejectedRatings.map(
                          (rating: any, index: number) => (
                            ...
                            >
                              <td className="text-left py-1 px-2">
                                {rating.email ? rating.email : 'No Value'}
                              </td>
                              <td className="text-left py-1 px-2">
                              ...
                              <td className="text-left py-1 px-2">
                                {rating.professional_skills !== null
              ...
              <div className="flex justify-between w-full my-2">
                <button
                  ...
                  type="button"
                  onClick={() => setBulkRateModal(false)}
                >
                  Cancel
                </button>
                <button
              ...
              {teams ? (
                <>
                  ...
                  <div className="flex flex-col gap-3 p-3 rounded-md text-sm bg-neutral-700">
                    <p className="">
                      Would you like to download a team template .xlsx file for
                      easier rating?
                    </p>
                    <div className="flex gap-2">
                      <select
                        data-testid="select-team"
                        className="p-1 text-sm text-black dark:text-white rounded-lg bg-white dark:bg-dark border-2 border-primary"
                        defaultValue=""
                        onChange={(e) => setSelectedTeam(e.target.value)}
                      >
                        <option data-testid="team-default-option">
                          Choose a team
                        </option>
                        {teams.getTeamsByCohort.length > 0 ? (
                          teams.getTeamsByCohort.map((team: any) => (
                            <option
                          ...
                          <option data-testid="team-empty-option" value="">
                            No teams found...
                          </option>
                        )}
                      </select>
                      <button
                        ...
                        type="button"
                        onClick={downloadTeamFile}
                        className="py-1 px-2 text-white rounded-lg bg-green-700 text-sm font-serif font-semibold"
                      >
                        Download
                      </button>
                    </div>
