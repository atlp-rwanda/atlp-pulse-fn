...
const GradingSystem = () => {
    ...
    const removeModel =/* istanbul ignore next */ () => {
        let newState =/* istanbul ignore next */ !addGradingSystemModel;
        /* istanbul ignore next */
    ...
    useEffect(() => {
        if (formData.label === 'custom') {
            ...
            fileteredData = dataValues.filter(
                (item: any) => /* istanbul ignore next */item.name === formData.label,
            );
    ...
    return (
        <>
            ...
            <MakeDefaultModal
                ...
                makeDefaultFunc={/* istanbul ignore next */() => makeRatingdefault()}
            />
            <div
                className={`h-screen w-screen z-20 bg-black bg-opacity-30 backdrop-blur-sm absolute flex items-center justify-center px-4 ${addGradingSystemModel === true ? 'block' : 'hidden'
                    }`}
            >
