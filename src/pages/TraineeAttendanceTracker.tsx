...
function TraineeAttendanceTracker() {
        ...
        if (user.role === 'coordinator') {
          getAllTeams({
            ...
            onCompleted: (data) => {
              ...
              setSelectedPhase(data.getAllTeams[0].phase || data.getAllTeams[0].cohort.phase);
        ...
        if (user.role === 'ttl') {
          getTTLTeams({
            ...
            onCompleted: (data) => {
              ...
              setSelectedPhase(data.getTTLTeams[0].phase || data.getTTLTeams[0].cohort.phase);
  ...
  useEffect(() => {
    ...
    selectedTeamId &&
      getTeamAttendance({
        ...
        onCompleted: (data) => {
          setSelectedPhase({
            id: selectedTeamData!.phase?.id || selectedTeamData!.cohort.phase.id,
            name: selectedTeamData!.phase?.name || selectedTeamData!.cohort.phase.name,
          });
    ...
    if (attendanceData) {
      ...
      const attendanceWeek = attendanceData.attendanceWeeks.filter(
        (attendanceWeek: any) => {
          ...
          return attendanceWeek.phase.id === selectedTeamData?.phase?.id || selectedTeamData?.cohort.phase.id;
        },
      );
  ...
  const submitAttendance = async () => {
    if (
      ...
      ((selectedTeamData?.phase?.id || selectedTeamData?.cohort.phase.id) === selectedPhase?.id)
    ) {
      openModal();
      return;
    }
