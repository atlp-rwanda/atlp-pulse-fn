    ...
    try {
      ...
      const { data } = await LoginUser({
        ...
        onCompleted: async (data) => {
          if (data.loginUser?.message.includes('Two-factor authentication')) {
            // 2FA is required, show the 2FA input field
            setShowTwoFactorInput(true);
            toast.info(data.loginUser.message);
          } else {
            // Handle the regular login flow
        ...
        onError: (err) => {
          if (
            err.message.toLowerCase() !== 'invalid credential' &&
            !err.message.includes('Two-factor authentication')
          ) {
          ...
          <div className="flex flex-col items-center">
            ...
            >
              ...
              <div className="md:w-full border border-gray rounded-md bg-gray-100 p-2 my-4 flex items-center mb-2 dark:border-white dark:bg-dark-bg">
                ...
                >
                  {passwordShown ? <FaRegEye /> : <FiEyeOff />}
                </div>
              </div>
